...
//桌機版手札推薦hover
// $('.blogRanking-event-wrap').mouseover(function(){
// 	$('.blogRanking-event-wrap').css('background-color','black');
// });
...
var arr = ["leftCardFrame","middleCardFrame","rightCardFrame"];  
var curIndex = 2;
var cardFrames = document.getElementsByClassName("cardFrame");
var cardLength = cardFrames.length;

window.addEventListener("load", function (){
	$id("leftBtn").onclick = function(){
		console.log('hi')
		for( var i=0; i<cardLength; i++){
			cardFrames[i].classList.remove(arr[i]);
		}
		arr.unshift(arr.pop());
		console.log(arr.toString());
		for( var i=0; i<cardLength; i++){
			cardFrames[i].classList.add(arr[i]);
		}    
	};
