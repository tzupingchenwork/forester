...
// }
document.getElementById("play").addEventListener("click", playchick);
...
function playchick() {
  ...
  xhr.open("get", "php/playchick.php", true);
  xhr.send(null);
  ...
  function showMember(msg) {
    if (msg == "登入") {
      ...
      var day = new Date();
      var pointDate=day.getFullYear()+'-'+(day.getMonth()+1)+'-'+day.getDate()
      console.log(typeof(pointDate));
      var xhr = new XMLHttpRequest();
      xhr.onload = function () {
        if (xhr.status == 200) {
          showMember(xhr.responseText);
      ...
      xhr.open("get", "php/eatbeecheckplay.php?pointDate="+pointDate, true);
      xhr.send(null);
...
continu.addEventListener("click", function () {
  ...
  window.open('', '_self', '');
  window.close();
  var point = parseInt(finalsorce.innerHTML);
  ...
  xhr.open("get", "php/eatbeeaddvalue.php?point=" + point+"&pointDate="+pointDate, true);
  xhr.send(null);
});